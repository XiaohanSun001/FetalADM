...
def performance(y_true, y_pred,y_pred_proba):
    ...
    return metric
path='../../data_400_beforeused/'
#filename=path+'primedata.csv'
filename=path+'smotedata.csv'
all_data = pd.read_csv(filename)

data,label = dataSplit(all_data)
test_data = pd.read_csv(path+'test46.csv')
...
if best_model is not None:
    ...
    out.write('Best Model Test Set Performance: \n')
    out.write('accuracy, precision_micro, precision_weighted, recall_micro, recall_weighted,  fl_micro, fl_weighted,mcc \n')
