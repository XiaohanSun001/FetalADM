...
reg = SVC(probability=True)

all_classes = np.unique(label)

# 定义分层交叉验证
stratified_kfold = StratifiedKFold(n_splits=5,random_state=6, shuffle=True)
...
for train_index, val_index in stratified_kfold.split(X_train, y_train):
    ...
    #print(len(y_val_fold), len(y_val_pred),len(y_val_pred_proba))
...
if best_model is not None:
    ...
    out.write('accuracy, precision_weighted,fl_micro, fl_weighted,mcc \n')
